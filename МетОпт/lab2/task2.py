...
def midpoint_method (a, b, eps = 0.01, minimum = True) -> pd.DataFrame:
    iterations = []
    step = 1
    while (b - a) > 2 * eps:
        c = (a + b) / 2
        dc = (1 if minimum else -1)*df(c)
...
if __name__ == "__main__":
    data_min = midpoint_method(*borders, eps=EPSILON, minimum=True)
    data_max = midpoint_method(*borders, eps=EPSILON, minimum=False)
