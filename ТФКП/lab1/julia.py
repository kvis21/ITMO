...
class JuliaSet:
    def __init__(self, width=800, height=800):
        ...
        self.x = np.linspace(-2, 2, width) # Вещественная ось от -2 до 2
        self.y = np.linspace(-2, 2, height) # Мнимая ось от -2 до 2
        self.X, self.Y = np.meshgrid(self.x, self.y) # Сетка координат
    ...
    def julia_iteration(self, z, c):
        """Одна итерация отображения Жюлиа: z -> z^2 + c"""
        return z**2 + c
    ...
    def compute_julia_set(self, c=None, max_iter=None):
        ...
        for i in range(max_iter):
            # Применяем отображение Жюлиа: f(z) = z^2 + c
